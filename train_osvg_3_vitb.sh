...
OUTPUT=outputs/${DATA_NAME}/osvg_clip_vitb16_${IMAGE_SIZE}_g${GPUS}_b${BATCH_SIZE}_freezetxt

srun -p ai4earth --gres=gpu:$GPUS --job-name=${DATA_NAME} --quotatype=reserved \
 python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28601 --use_env train_osvg_3.py \
 ...
 --output_dir ${OUTPUT} --device cuda --resume ${OUTPUT}/checkpoint.pth --seed 14

srun -p ai4earth --gres=gpu:$GPUS --job-name=$DATA_NAME --quotatype=reserved python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28601 --use_env eval_osvg.py \
