...
OUTPUT=outputs/unc/osvg_clip_vitl_${IMAGE_SIZE}_g${GPUS}_b${BATCH_SIZE}

srun -p ai4earth --gres=gpu:$GPUS --job-name=l_$DATA_NAME --quotatype=reserved \
 python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 23214 --use_env train_osvg.py \
...
srun -p ai4earth --gres=gpu:$GPUS --job-name=$DATA_NAME --quotatype=reserved python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 23214 --use_env eval_osvg.py \
