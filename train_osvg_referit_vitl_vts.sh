...
# ReferItGame
srun -p ai4earth --gres=gpu:$GPUS --job-name=l_${DATA_NAME}${CE_KEEP} --quotatype=reserved \
 python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28522 --use_env train_osvg.py \
