...
# srun -p ai4earth --gres=gpu:$GPUS --job-name=b${DATA_NAME}${CE_KEEP} --quotatype=reserved \
#  python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 27763  --use_env train_osvg.py \
...
srun -p ai4earth --gres=gpu:$GPUS --job-name=$DATA_NAME --quotatype=spot python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 27763 --use_env eval_osvg.py \
...
# srun -p ai4earth --gres=gpu:$GPUS --job-name=$DATA_NAME --quotatype=reserved python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 27763 --use_env eval_osvg.py \
#  --num_workers $UNM_WORKERS --batch_size $BATCH_SIZE  --dataset $DATA_NAME  --model $MODEL  --imsize $IMAGE_SIZE --max_query_len 77 --ce --ce_keep $CE_KEEP \
#  --data_root image_data --split_root split_data --eval_model ${OUTPUT}/best_checkpoint.pth  \
