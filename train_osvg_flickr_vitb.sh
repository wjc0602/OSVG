...
srun -p ai4earth --gres=gpu:$GPUS --job-name=$DATA_NAME --quotatype=reserved python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28888 --use_env eval_osvg.py \
...
# srun -p ai4earth --gres=gpu:2 --job-name=vg_test --quotatype=spot python -m torch.distributed.launch --nproc_per_node=2 --master_port 28888 --use_env eval_osvg_clip.py \
#  --num_workers $UNM_WORKERS --batch_size $BATCH_SIZE  --dataset $DATA_NAME  --imsize $IMAGE_SIZE --max_query_len 77 \
#  --data_root image_data --split_root split_data --eval_model outputs/${OUTPUT}/best_checkpoint.pth  \
#  --eval_set testA --device cuda --output_dir outputs/${OUTPUT}

# srun -p ai4earth --gres=gpu:2 --job-name=vg_test --quotatype=spot python -m torch.distributed.launch --nproc_per_node=2 --master_port 28888 --use_env eval_osvg_clip.py \
#  --num_workers $UNM_WORKERS --batch_size $BATCH_SIZE  --dataset $DATA_NAME  --imsize $IMAGE_SIZE --max_query_len 77 \
#  --data_root image_data --split_root split_data --eval_model outputs/${OUTPUT}/best_checkpoint.pth  \
#  --eval_set testB --device cuda --output_dir outputs/${OUTPUT}