...
#  python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28567  --use_env train_osvg.py \
...
srun -p ai4earth --gres=gpu:$GPUS --job-name=$DATA_NAME --quotatype=spot python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28567 --use_env eval_osvg.py \
