...
class DynamicHead(nn.Module):
    ...
    def __init__(self, num_classes, roi_input_shape):
        super().__init__()
    ...
    def _init_box_pooler(input_shape):
        ...
        pooler_resolution = 7
        pooler_scales = [1.0 / 16.0]
        sampling_ratio = 2
        pooler_type = 'ROIAlignV2'

        # If StandardROIHeads is applied on multiple feature maps (as in FPN),
        # then we share the same predictors and therefore the channel counts must be the same
        in_channels = [768 for f in range(len(input_shape))]
...
class RCNNHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        roi_features = pooler([features], proposal_boxes) #[9600, 512, 7, 7]

        if pro_features is None:
            pro_features = roi_features.view(N, nr_boxes, self.d_model, -1).mean(-1) # [32, 300, 512]

        roi_features = roi_features.view(N * nr_boxes, self.d_model, -1).permute(2, 0, 1) # [49, 9600, 512]
