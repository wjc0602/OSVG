...
def ceblock_forward(block, x, lens_t, keep_ratio_search=None, global_index=None):

    x_attn, attn = block.attention(block.ln_1(x))
    x = x + x_attn # [197, 32, 768]
    x = x.permute(1,0,2)
    x, global_index_template, removed_index_search = candidate_elimination(attn, x, lens_t, keep_ratio_search, global_index)
    x = x.permute(1,0,2)

    x = x + block.mlp(block.ln_2(x)) # [197, 32, 768]
    return x, global_index_template, removed_index_search, attn
...
class MultiLevel_Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, lens_t, ce_keep_rate, ce_loc):
        ...
        for i in range(max(self.extract_layer)+1):
            if ce_keep_rate <1 and i in ce_loc:
                x, global_index_template, removed_index_search, attn = ceblock_forward(self.resblocks[i], x, lens_t, ce_keep_rate, global_index_s)
                removed_indexes_s.append(removed_index_search)
            else:
                x = self.resblocks[i](x)
        return x, removed_indexes_s
...
class MultiLevel_ImageEncoder_modified(nn.Module):
    ...
    def forward(self, x, text_tensors, ce_keep_rate, ce_loc):
        ...
        x, removed_indexes_s = self.transformer(x, txt_len, ce_keep_rate, ce_loc)
        cls_token = x[0, :, :] # split txt features
        return cls_token, removed_indexes_s
...
class OSVG(nn.Module):
    ...
    def forward(self, img_data, text_data, ce_keep_rate):
        ...
        visu_src, removed_indexes_s = self.image_encoder_clip_vg(image_tensors.type(self.clip.dtype), self.text_resize(text_features), ce_keep_rate, self.ce_layer)  # B * 197 * 512 
        visu_src = self.neck(visu_src)  # B 4H -> B H
        pred_box = self.bbox_embed(visu_src).sigmoid()
        return pred_box, removed_indexes_s
