...
DATA_NAME=unc+
OUTPUT=outputs/${DATA_NAME}/osvg_clip_vitb16_${IMAGE_SIZE}_g${GPUS}_b${BATCH_SIZE}_c${CE_KEEP}

srun -p ai4earth --gres=gpu:$GPUS --job-name=${DATA_NAME}${CE_KEEP} --quotatype=reserved \
 python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28533  --use_env train_osvg.py \
...
srun -p ai4earth --gres=gpu:$GPUS --job-name=$DATA_NAME --quotatype=reserved python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port 28533 --use_env eval_osvg.py \
 ...
 --data_root image_data --split_root split_data --eval_model ${OUTPUT}/checkpoint.pth  \
